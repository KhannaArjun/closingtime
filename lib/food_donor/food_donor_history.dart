...
class FoodDonorHistory extends StatefulWidget {
  const FoodDonorHistory({Key? key}) : super(key: key);

  @override
  _FoodDonorHistoryState createState() => _FoodDonorHistoryState();
}

class _FoodDonorHistoryState extends State<FoodDonorHistory> {
  ...
  Widget _itemCard(BuildContext context, Data addedFoodModel)
  {
    return SizedBox(
      ...
      child: Card(
        ...
        child: InkWell(
          onTap: () async {
            var result = await Navigator.of(context).push(MaterialPageRoute(builder: (context) => DonorFoodDescription(addedFoodModel, true)));
            if (result == true)
            {
              //getUserId();
            }
  ...
  async {
    ...
    try
    {
      Future<AddedFoodListModel> addedFoodListModel = ApiService.getFoodHistoryByDonor(jsonEncode(body));
